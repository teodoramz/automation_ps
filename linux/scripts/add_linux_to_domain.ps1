...
foreach ($vm in $vms) {
    try{
        ...
        $cmdUpdatePackages = "echo `'$password`' | sudo -S apt-get update -y"

        $cmdInstallDependencies = "echo `'$password`' | sudo -S apt-get install -y realmd sssd sssd-tools adcli krb5-user packagekit samba-common-bin"

        # to add a timeout after 30 sec to do sudo dpkg --configure -a"

        # Command for adding the computer to a domain
        $cmdDomain = "echo `'$rootPassword`' | sudo -S bash -c `"echo '$domainPassword' | realm join -U $domainUser $domainName`""


        # Command for rebooting the system
        $cmdReboot = "echo `'$rootPassword`' | sudo -S reboot"
        ...
        $scriptResult = Invoke-VMScript -VM $vmObject -ScriptType Bash -ScriptText $cmdUpdatePackages -Verbose -GuestCredential $cred
        $scriptOutput = $scriptResult.ScriptOutput
        Write-Host "Script output: $scriptOutput"

        # Install dependencies for joining domain
        $scriptResult = Invoke-VMScript -VM $vmObject -ScriptType Bash -ScriptText $cmdInstallDependencies -Verbose -GuestCredential $cred
        $scriptOutput = $scriptResult.ScriptOutput
        Write-Host "Script output: $scriptOutput"

        # Adding the computer to the domain
        #
        #  ( check update_network_hostname_unix.ps1 if you want to change the computer's hostname before adding it to the domain )
        #                           ( or give other hostname through realm join command )
        #
        $scriptResult = Invoke-VMScript -VM $vmObject -ScriptType Bash -ScriptText $cmdDomain -Verbose -GuestCredential $cred
        $scriptOutput = $scriptResult.ScriptOutput
        Write-Host "Script output: $scriptOutput"
